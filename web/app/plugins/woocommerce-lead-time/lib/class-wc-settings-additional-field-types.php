...
if ( ! class_exists( 'WC_Settings_Additional_Field_Types' ) ) {
	 ...
	 * @version   1.4
	 * @deprecated Replaced by Barn2\WLT_Lib\WooCommerce\Admin\Custom_Settings_Fields;
	 */
	class WC_Settings_Additional_Field_Types {

		public static function register_settings() {
			foreach ( [ 'color_size', 'help_note', 'hidden', 'multi_text', 'settings_start', 'settings_end', 'plugin_promo' ] as $field ) {
				if ( ! has_action( "woocommerce_admin_field_{$field}" ) ) {
					add_action( "woocommerce_admin_field_{$field}", [ 'WC_Settings_Additional_Field_Types', "{$field}_field" ] );
				}
			}
		}
