<?php
namespace Barn2\WLT_Lib\Plugin\License\Admin;

use Barn2\WLT_Lib\Registerable,
	Barn2\WLT_Lib\Plugin\License\License,
	Barn2\WLT_Lib\Admin\Settings_API_Helper,
	WC_Admin_Settings;
...
class License_Key_Setting implements Registerable, License_Setting {
		...
		if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
			...
			if ( isset( $license_setting['license'] ) ) {
				...
				$this->add_settings_message(
					__( 'License key successfully activated.', 'woocommerce-lead-time' ),
					__( 'There was an error activating your license key.', 'woocommerce-lead-time' ),
					$activated
				);
			}
			...
			$this->add_settings_message(
				__( 'License key successfully deactivated.', 'woocommerce-lead-time' ),
				__( 'There was an error deactivating your license key, please try again.', 'woocommerce-lead-time' ),
				$deactivated
			);
			...
			$this->add_settings_message(
				__( 'The license key looks good!', 'woocommerce-lead-time' ),
				__( 'There\'s a problem with your license key.', 'woocommerce-lead-time' ),
	...
	public function get_license_key_setting() {
		$setting = [
			'title' => __( 'License key', 'woocommerce-lead-time' ),
			'type'  => 'text',
		...
		if ( $this->is_woocommerce ) {
			$setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'woocommerce-lead-time' );
		} elseif ( $this->is_edd ) {
			// EDD uses title case for setting names, so let's keep things consistent.
			$setting['title'] = __( 'License Key', 'woocommerce-lead-time' );
	...
	private function get_license_description() {
		$buttons = [
			...
			'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'woocommerce-lead-time' ) ),
			'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'woocommerce-lead-time' ) )
		];
		...
		if ( $this->license->is_active() ) {
			...
			if ( $this->license->is_inactive() && $this->is_license_action( 'deactivate_key' ) ) {
				...
				$message = __( 'License key deactivated.', 'woocommerce-lead-time' );
			} else {
