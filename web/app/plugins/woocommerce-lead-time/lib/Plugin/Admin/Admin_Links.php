...
namespace Barn2\WLT_Lib\Plugin\Admin;

use Barn2\WLT_Lib\Registerable,
	Barn2\WLT_Lib\Service,
	Barn2\WLT_Lib\Plugin\Plugin,
	Barn2\WLT_Lib\Util;
...
class Admin_Links implements Registerable, Service {
	...
	public function add_settings_link( $links ) {
		...
		array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'woocommerce-lead-time' ) ) );
		return $links;
	}
	...
	public function add_documentation_link( $links, $file ) {
		...
		$row_meta = [
			'docs' => sprintf(
				...
				esc_attr( sprintf( __( 'View %s documentation', 'woocommerce-lead-time' ), $this->plugin->get_name() ) ),
				esc_html__( 'Docs', 'woocommerce-lead-time' )
			)
		];
