...
class GoogleAds extends Settings implements Pixel {
	...
	public function configured() {
        ...
        $ads_ids = $this->getAllActivePixels();
    ...
    public function getAllActivePixels() {
        $pixels = $this->getPixelIDs();
            ...
            && SuperPack()->getOption( 'additional_ids_enabled' ))
        {
            ...
            foreach ($additionalPixels as $_pixel) {
                if($this->isValidForCurrentLang($_pixel)) {
                    $pixels[]=$_pixel->pixel;
                }
	...
	public function getPixelOptions() {
		...
		return array(
			'conversion_ids'      => $this->getAllActivePixels(),
    ...
    public function generateEvents($event) {
        ...
        $disabledPixel =  (array)apply_filters( 'pys_pixel_disabled', '', $this->getSlug() );
        if($event->getId() == 'custom_event') {
            ...
            if ( $conversion_id == '_all' ) {
                $ids = $this->getAllPixelsForEvent($event);
                if(count($ids) > 0) {
                    $conversion_id = $ids[0];
                }
            }
            // AW-12345678 => AW-12345678/da324asDvas
            if(!in_array($disabledPixel,$disabledPixel)) {
