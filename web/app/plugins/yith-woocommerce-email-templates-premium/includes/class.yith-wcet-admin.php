	...
	class YITH_WCET_Admin {
		...
		public function post_type_register() {
			$labels = array(
				...
				'view'               => __( 'View Email Template', 'yith-booking-for-woocommerce' ),
				...
				'item_updated'       => __( 'Email template updated', 'yith-woocommerce-email-templates' ),
				'new_item'           => __( 'New Email Template', 'yith-woocommerce-email-templates' ),
				'search_items'       => __( 'Search Email Template', 'yith-woocommerce-email-templates' ),
				'parent'             => __( 'Parent Email Templates', 'yith-woocommerce-email-templates' ),
				'menu_name'          => _x( 'Email Templates', 'Admin menu name', 'yith-woocommerce-email-templates' ),
				'all_items'          => __( 'All Email Templates', 'yith-woocommerce-email-templates' ),
			);
		...
		public function post_updated_messages( $messages ) {
			...
			$messages['yith-wcet-etemplate'] = array(
				0  => '', // Unused. Messages start at index 1.
				1  => __( 'Email template updated', 'yith-woocommerce-email-templates' ),
				2  => __( 'Custom field updated.', 'yith-woocommerce-email-templates' ),
				3  => __( 'Custom field deleted.', 'yith-woocommerce-email-templates' ),
				4  => __( 'Email template updated.', 'yith-woocommerce-email-templates' ),
				5  => __( 'Revision restored.', 'yith-woocommerce-email-templates' ),
				6  => __( 'Email template published', 'yith-woocommerce-email-templates' ),
				7  => __( 'Email template saved.', 'yith-woocommerce-email-templates' ),
				8  => __( 'Email template submitted.', 'yith-woocommerce-email-templates' ),
				9  => sprintf(
				/* translators: %s is the date */
					__( 'Email template scheduled for: %s.', 'yith-woocommerce-email-templates' ),
					'<strong>' . date_i18n( wc_date_format() . ' @ ' . wc_time_format(), strtotime( $post->post_date ) )
				),
				10 => __( 'Email template draft updated.', 'yith-woocommerce-email-templates' ),
			);
		...
		public function action_links( $links ) {
			return yith_add_action_links( $links, $this->_panel_page, defined( 'YITH_WCET_PREMIUM' ), YITH_WCET_SLUG );
		}
		...
		public function admin_enqueue_scripts() {
			...
			wp_register_script( 'yith-wcet-popup', YITH_WCET_ASSETS_URL . "/js/yith-wcet-popup{$suffix}.js", array( 'jquery', 'jquery-blockui' ), YITH_WCET_VERSION );

			if ( 'yith-wcet-etemplate' === $screen_id ) {
				...
				wp_enqueue_script( 'yith_wcet_meta_box', YITH_WCET_ASSETS_URL . "/js/meta-box{$suffix}.js", array( 'jquery', 'wp-color-picker', 'jquery-blockui', 'wp-codemirror', 'yith-wcet-popup' ), YITH_WCET_VERSION, true );
				wp_localize_script( 'yith_wcet_meta_box', 'yith_wcet_meta_box', array( 'assets_url' => YITH_WCET_ASSETS_URL ) );

				wp_enqueue_style( 'yith-wcet-popup' );
			}
			wp_register_script( 'yith-wcet-admin-js', YITH_WCET_ASSETS_URL . "/js/admin{$suffix}.js", array( 'select2' ), YITH_WCET_VERSION, true );

			if ( strpos( $screen_id, 'page_yith_wcet_panel' ) > 0 || strpos( $screen_id, '_page_wc-settings' ) ) {
				wp_enqueue_style( 'yith-wcet-admin-styles' );
